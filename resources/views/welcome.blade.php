...
<main>
    ...
    <section id="featured" class="site-shell section-block"><div class="section-heading"><div><p class="eyebrow">{{ __('ui.home.featured_eyebrow') }}</p><h2>{{ __('ui.home.featured_title') }}</h2></div><a class="text-link" href="{{ Route::has('novels.index') ? route('novels.index') : '#' }}">{{ __('ui.home.all_featured') }} <span>→</span></a></div><div class="featured-grid">@forelse ($featured as $index => $book)<article class="featured-card {{ $index === 0 ? 'featured-card-primary' : '' }}"><a href="{{ route('novels.show', ['novel' => $book['slug']]) }}" class="cover-link"><x-book-cover :book="$book" size="large" /></a><div class="featured-card-copy"><p class="book-kicker">{{ $book['genre'] }}</p><h3><a href="{{ route('novels.show', ['novel' => $book['slug']]) }}">{{ $book['title'] }}</a></h3><p>{{ $book['desc'] ?? __('ui.novel_detail.default_description') }}</p><div class="book-byline"><span>{{ $book['author'] }}</span><span>{{ __('ui.home.updated_to', ['count' => $book['chapters']]) }}</span></div></div></article>@empty<p class="muted">{{ __('ui.library.empty_intro') }}</p>@endforelse</div></section>
