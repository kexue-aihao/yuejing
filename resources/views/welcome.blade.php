...
<main>
    ...
    <section id="featured" class="site-shell section-block"><div class="section-heading"><div><p class="eyebrow">精选阅读</p><h2>这一刻，值得翻开</h2></div><a class="text-link" href="{{ Route::has('novels.index') ? route('novels.index') : '#' }}">全部精选 <span>→</span></a></div><div class="featured-grid">@foreach ($featured as $index => $book)<article class="featured-card {{ $index === 0 ? 'featured-card-primary' : '' }}"><a href="{{ Route::has('novels.show') ? route('novels.show', ['novel' => $book['slug']]) : '#' }}" class="cover-link"><x-book-cover :book="$book" size="large" /></a><div class="featured-card-copy"><p class="book-kicker">{{ $book['genre'] }}</p><h3><a href="{{ Route::has('novels.show') ? route('novels.show', ['novel' => $book['slug']]) : '#' }}">{{ $book['title'] }}</a></h3><p>{{ $book['desc'] }}</p><div class="book-byline"><span>{{ $book['author'] }}</span><span>{{ $book['chapters'] }} 章 · {{ $book['updated'] }}</span></div></div></article>@endforeach</div></section>
    <section class="site-shell section-block section-divider"><div class="section-heading"><div><p class="eyebrow">刚刚上新</p><h2>新的故事，新的入口</h2></div><a class="text-link" href="{{ Route::has('novels.index') ? route('novels.index', ['sort' => 'new']) : '#' }}">发现更多 <span>→</span></a></div><div class="new-books-grid">@foreach ($newBooks as $book)<article class="book-row"><a href="{{ Route::has('novels.show') ? route('novels.show', ['novel' => $book['slug']]) : '#' }}"><x-book-cover :book="$book" size="small" /></a><div class="book-row-copy"><p class="book-kicker">{{ $book['genre'] }}</p><h3><a href="{{ Route::has('novels.show') ? route('novels.show', ['novel' => $book['slug']]) : '#' }}">{{ $book['title'] }}</a></h3><p class="muted">{{ $book['author'] }}</p><span class="chapter-count">更新至 {{ $book['chapters'] }} 章</span></div></article>@endforeach</div></section>
